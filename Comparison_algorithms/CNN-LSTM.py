...
Testalphafile0 = r"neg_Train_x.mat"
TestData0 = sio.loadmat(Testalphafile0)['Train_x']
...
TrainAlphafile0 = r"neg_Test_x.mat"
TrainAlpha0 = sio.loadmat(TrainAlphafile0)
x_train0 = TrainAlpha0['Test_x']  # (1428, 500)
...
def create_model(alpha_k1, alpha_k2, sem_k1, sem_k2, lstm_us):
    ...
    model_m_alpha.add(LSTM(units=lstm_us, return_sequences=False))
    ...
    model_m_sem.add(LSTM(units=lstm_us, return_sequences=False))
    model_m_sem.add(Dropout(0.2))  # 添加全局平均池化层来减少维度
...
results_df.to_excel('gjjCNNLSTMATTENTION_parameter_optimization_results.xlsx', index=False)
...
# 保存最佳结果到Excel
with pd.ExcelWriter('CNNLSTM_parameter_optimization_results.xlsx', mode='a', engine='openpyxl') as writer:
    test_results.to_excel(writer, sheet_name='Test Results', index=False)

print(f"Test results saved to 'gjjCNNLSTMATTENTION_parameter_optimization_results.xlsx'.")
