...
from keras.layers import Conv1D, MaxPooling1D, BatchNormalization, Concatenate
...
results_df.to_excel('gjjCNN_parameter_optimization_results.xlsx', index=False)
...
# 保存最佳结果到Excel
with pd.ExcelWriter('CNN_parameter_optimization_results.xlsx', mode='a', engine='openpyxl') as writer:
    test_results.to_excel(writer, sheet_name='Test Results', index=False)

print(f"Test results saved to 'gjjCNN_parameter_optimization_results.xlsx'.")
